...
class EpisodeRunner:
    ...
    def run(self, mac, test_mode=False, EA=False, MultiObject=False, phase=0, **kwargs):
        self.mac = mac
        self.reset()
        ...
        while not terminated:
            ...
            if self.args.Pareto and MultiObject:
                result = self.env.step(actions[0], MultiObject=MultiObject)

                # 检查返回值的数量
                try:
                    reward, terminated, env_info, fitness = result
                except:
                    print(result, "4to3_error")
                # reward, terminated, env_info, fitness = self.env.step(actions[0], MultiObject=MultiObject)
                for i in range(len(fitness)):
                    fitnesseslist[i] += fitness[i]
            else:
        ...
        if self.args.Pareto:
            return self.batch, fitnesseslist, env_info.get("battle_won", 0)
        else:
